...
describe('API: Authentication', () => {
    ...
    describe('GET /authentication ? key=:key & passcode=:pc', function(){
        ...
        it('should fail when key doesn\'t exist', async function(){
            let { status, body } = await get(ENDPOINT + '?key=testo&passcode=googlecom');
        ...
        it('should fail when passcode doesn\'t match stored one', async function(){
            let { status, body } = await get(ENDPOINT + '?key=test&passcode=googlecom');
            assert.equal(status, 200);
            let o = JSON.parse(body);
            assert.equal(o.result, 'failure');
        });

        it('should succeed when key and passcode are fine', async function(){
            let { status, body } = await get(ENDPOINT + '?key=test&passcode=google.com');
            assert.equal(status, 200);
            let o = JSON.parse(body);
